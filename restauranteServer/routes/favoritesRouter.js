...
.post(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
    Favorites.findOne({'user': req.user._id}, (err, favorites) => {
        ...
        if (favorites == null) {
            ...
            .then((favoriteDishes) => {
                console.log('Favorite Dishes Added ', util.inspect(favoriteDishes, {showHidden: false, depth: null}));
                res.statusCode = 200;
                res.setHeader('Content-Type', 'application/json');
...
.post(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
    Favorites.findOne({'user': req.user._id}, (err, favorites) => {
        ...
        if (favorites == null) {
            ...
            .then((favoriteDishes) => {
                console.log('Favourite Dishes Added ', util.inspect(favoriteDishes, {showHidden: false, depth: null}));
                res.statusCode = 200;
                res.setHeader('Content-Type', 'application/json');
                res.json(favoriteDishes);
            }, (err) => next(err))
